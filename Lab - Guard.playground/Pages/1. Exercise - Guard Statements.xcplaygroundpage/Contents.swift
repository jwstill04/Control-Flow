...
func add(int1: Int?, int2: Int?) -> Int? {
    guard let firstInt = int1, let secondInt = int2 else {
        return nil
    }
...
let john: User? = createUser()

if let newUser = john {
    print("The user's first name is \(newUser.firstName), last name is \(newUser.lastName), and age is \(newUser.age).")
}
/*:
